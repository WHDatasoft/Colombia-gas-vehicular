...
import { closeSession } from '../../cognito/authFunctions'

const NavAdmin = ({ setEnableNav, enableNav }) => {
	...
	const linkList = [
		{ title: 'Publicidad', url: '/admin' },
		{
			title: 'Servicios', url: '/admin/',
			list: [
				{ title: 'instalación GNV', url: '/admin/servicios/instalacion-gnv' },
				{ title: 'Mantenimiento', url: '/admin/servicios/mantenimiento' },
				{ title: 'Revisión anual', url: '/admin/servicios/revision-anual' },
				{ title: 'Prueba hidrostática', url: '/admin/servicios/prueba-hidrostatica' },
				{ title: 'venta de repuestos', url: '/admin/servicios/venta-de-repuestos' },
			]
		},
	...
	return <div className="wrapper">
			...
			<ul className="main-link-list">
				{
					linkList.map(mainLink => (
						<li key={mainLink.title} className={`main-link ${linkSelected === mainLink.url || '/' + router.asPath.split('/')[2] === mainLink.url ? 'selected-first' : ''}`} >
							<>
								{
									submenu
										?
				...
				<li className="main-link">
					<button onClick={() => closeSession(router)}>
						Cerrar Sesión
					</button>
				</li>
		...
		<style jsx>{`
			...
			.main-link-list {
				display: grid;
				grid-template-columns: repeat(${linkList.length + 1}, auto);
				height: 100%;
